// -*- C++ -*-
//
// Package:    Analyzer/RecHitAnalyzer
// Class:      RecHitAnalyzer
//
/**\class RecHitAnalyzer RecHitAnalyzer.cc MLAnalyzer_run/RecHitAnalyzer/plugins/RecHitAnalyzer.cc
...
RecHitAnalyzer::RecHitAnalyzer(const edm::ParameterSet& iConfig)
{
  ...
  trackMode_ = iConfig.existsAs<std::string>("trackMode") ? iConfig.getParameter<std::string>("trackMode") : "all";
  ...
  edm::InputTag ak8JetCollectionTag("ak8PFJets");
  if (iConfig.existsAs<edm::InputTag>("ak8PFJetCollection")) {
    ak8JetCollectionTag = iConfig.getParameter<edm::InputTag>("ak8PFJetCollection");
  }
  ak8JetCollectionT_      = consumes<reco::PFJetCollection>(ak8JetCollectionTag);
  ...
  doJetHitMask_        = iConfig.existsAs<bool>("doJetHitMask")        ? iConfig.getParameter<bool>("doJetHitMask")        : false;
  jetHitMaskUseAK4_    = iConfig.existsAs<bool>("jetHitMaskUseAK4")    ? iConfig.getParameter<bool>("jetHitMaskUseAK4")    : true;
  jetHitMaskUseAK8_    = iConfig.existsAs<bool>("jetHitMaskUseAK8")    ? iConfig.getParameter<bool>("jetHitMaskUseAK8")    : false;
  jetHitMaskConeAK4_   = iConfig.existsAs<double>("jetHitMaskConeAK4") ? iConfig.getParameter<double>("jetHitMaskConeAK4") : 0.4;
  jetHitMaskConeAK8_   = iConfig.existsAs<double>("jetHitMaskConeAK8") ? iConfig.getParameter<double>("jetHitMaskConeAK8") : 0.8;
  jetHitMaskMinJetPt_  = iConfig.existsAs<double>("jetHitMaskMinJetPt") ? iConfig.getParameter<double>("jetHitMaskMinJetPt") : minJetPt_;
  jetHitMaskMaxJetEta_ = iConfig.existsAs<double>("jetHitMaskMaxJetEta") ? iConfig.getParameter<double>("jetHitMaskMaxJetEta") : maxJetEta_;
  jetHitMaskMaxJets_   = iConfig.existsAs<int>("jetHitMaskMaxJets")    ? iConfig.getParameter<int>("jetHitMaskMaxJets")    : -1;

  std::cout << " >> Mode set to " << mode_ << std::endl;
  std::cout << " >> Track mode set to " << trackMode_ << std::endl;
  if ( mode_ == "JetLevel" ) {
    doJets_ = true;
  ...
  // Calorimeter, jet, and scalar branches -- always filled
  ...
  branchesJetHitMask ( RHTree, fs);

  // Per-jet constituent branches for ParT training (btvNano and fullTracks modes)
  if ( trackMode_ == "btvNano" || trackMode_ == "fullTracks" ) {
    branchesJetConstituents( RHTree, fs );
  }

  // PF candidate tracks -- filled in all track modes
  branchesPFCandsAtEBEE(RHTree, fs);
  branchesPFCandsAtECALstitched( RHTree, fs);

  // General tracks -- filled in fullTracks and all modes
  if ( trackMode_ == "fullTracks" || trackMode_ == "all" ) {
    branchesTracksAtEBEE(RHTree, fs);
    branchesTracksAtECALstitched( RHTree, fs);
    branchesTrackTripletsAtECAL( RHTree, fs);
  }

  // Tracker rechits, triplets, layers -- filled in all mode only
  if ( trackMode_ == "all" ) {
...
RecHitAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
 ...
 // Apply event selection cuts
  ...
  // Calorimeter, jet, and scalar fills -- always run
  ...
  fillJetHitMask( iEvent, iSetup );
  fillJetInfoAtECALstitched( iEvent, iSetup );
  fillScalarInfo( iEvent, iSetup );

  // Per-jet constituent fills for ParT training (btvNano and fullTracks modes)
  if ( trackMode_ == "btvNano" || trackMode_ == "fullTracks" ) {
    fillJetConstituents( iEvent, iSetup );
  }

  // PF candidate tracks -- all track modes
  fillPFCandsAtEBEE( iEvent, iSetup );
  fillPFCandsAtECALstitched( iEvent, iSetup );

  // General tracks -- fullTracks and all modes
  if ( trackMode_ == "fullTracks" || trackMode_ == "all" ) {
    fillTracksAtEBEE( iEvent, iSetup );
    for (unsigned int i=0;i<Nproj;i++)
    {
      fillTracksAtECALstitched( iEvent, iSetup, i );
      fillTrackTripletsAtECAL( iEvent, iSetup, i );
    }
  }
  ...
  if ( trackMode_ == "all" ) {
    ...
    for (unsigned int i=0;i<Nhitproj;i++)
    {
      ...
      fillTRKTriplets( iEvent, iSetup, i );
    }
  }
