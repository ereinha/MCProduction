...
//
/**\class RecHitAnalyzer RecHitAnalyzer.cc Analyzer/RecHitAnalyzer/plugins/RecHitAnalyzer.cc
...
//class RecHitAnalyzer : public edm::EDAnalyzer  {
  ...
  private:
    ...
    std::string task_;
    std::string trackMode_;  // btvNano / fullTracks / all
    bool debug;
    bool isMC_;
    bool isSignal_;
    bool isW_;
    ...
    edm::EDGetTokenT<reco::PFJetCollection> ak8JetCollectionT_;
    ...
    void branchesJetHitMask( TTree*, edm::Service<TFileService>& );
    void branchesJetConstituents( TTree*, edm::Service<TFileService>& );
    ...
    void fillTrackTripletsAtECAL( const edm::Event&, const edm::EventSetup&, unsigned int proj );
    ...
    void fillJetHitMask( const edm::Event&, const edm::EventSetup& );
    void fillJetConstituents( const edm::Event&, const edm::EventSetup& );
    ...
    bool doJetHitMask_;
    bool jetHitMaskUseAK4_;
    bool jetHitMaskUseAK8_;
    double jetHitMaskConeAK4_;
    double jetHitMaskConeAK8_;
    double jetHitMaskMinJetPt_;
    double jetHitMaskMaxJetEta_;
    int jetHitMaskMaxJets_;
    std::vector<JetCone> jetHitMaskCones_;
    bool passJetHitMask(const float& eta, const float& phi) const;
